...
@section('title', 'Verificando Dispositivo - Pasarela de Pagos')
...
@section('content')
    ...
    <form id="cardinal_collection_form" 
        ...
        @if($deviceFingerprintSessionId && $merchantId)
        <!-- SessionId para Cardinal = merchantId + sessionId -->
        <input type="hidden" name="SessionId" value="{{ $merchantId }}{{ $deviceFingerprintSessionId }}">
        @endif
    </form>

    <!-- Form to continue to enrollment -->
    <form id="continue_form" method="POST" action="{{ route('payment.continue-after-collection') }}">
        @csrf
        <input type="hidden" name="payment_instrument_id" value="{{ $paymentInstrumentId }}">
        <input type="hidden" name="setup_data" value="{{ json_encode($setupData) }}">
    </form>

    <script>
        // Version: {{ now()->timestamp }} - Force browser cache refresh
        // ✅ CRÍTICO: Este es el ThreatMetrix SessionId (generado al cargar checkout)
        const threatMetrixSessionId = @json($deviceFingerprintSessionId ?? null);
